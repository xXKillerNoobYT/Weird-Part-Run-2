...
struct NotebooksServiceTests {
    ...
    func testCreateBlockEntryPreservesPaletteBlockMetadata() throws {
        ...
        let checklistJson = "[{\"text\":\"Verify outlet labels\",\"checked\":\"false\"}]"
        let panelJson = "{\"panelType\":\"main\",\"circuits\":[{\"position\":1,\"label\":\"Kitchen\"}]}"
        let tableJson = "{\"columns\":[\"Part\",\"Qty\"],\"rows\":[[\"EMT\",\"12\"]]}"
        ...
        let checklistId = try env.notebooks.createBlockEntry(
            sectionId: sectionId,
            blockType: "checklist",
            title: "Punch",
            blockData: checklistJson,
        ...
        let hierarchy = try env.notebooks.getNotebookHierarchy(notebookId: nbId)
        let entries = try #require(hierarchy.ungroupedSections.first?.entries)
        let byId = Dictionary(uniqueKeysWithValues: entries.map { ($0.id, $0) })

        #expect(byId[headingId]?.blockType == "heading")
        #expect(byId[headingId]?.headingLevel == 2)
        #expect(byId[checklistId]?.blockType == "checklist")
        #expect(byId[checklistId]?.checklistItems == checklistJson)
        ...
        #expect(byId[tableId]?.blockData == tableJson)
        ...
        let detailTodo = try #require(try env.notebooks.getNotebookDetail(id: nbId).entries.first { $0.id == todoId })
