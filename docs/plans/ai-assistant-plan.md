...
18. Replacing the authenticated owner or `AppDatabase` is a privacy boundary: the mounted panel synchronously clears its visible transcript, saved Resume rows, active conversation ID, prior resume-attempt state, and retry errors before querying the replacement scope. History Retry starts only with positive owner/database prerequisites and validates an owner/database/conversation/revision token through completion; an older Retry cannot clear a replacement scope's loading gate.
19. Withdrawing the conversation-read database or owner while a Resume-list request is active cancels and retires that request, stops the picker spinner, and preserves both the last-good rows and the prior retryable error. Starting Retry may cover that error with loading UI, but only a matching successful replacement read clears it; if prerequisites disappear again, the preserved error and Retry control become reachable instead of leaving a spinner or dead end.
...
4. The assistant body renders visible, labeled controls with minimum 44×44-point hit regions while the Resume sheet is closed; while Resume is presented, that sheet renders the controls instead. This keeps exactly one actionable accessibility surface on screen. The controls include an accessibility-observable completion/error state identifier and are never transparent, hidden, coordinate-driven, or overlaid invisibly.
...
5. After a successful Resume-list load establishes last-known rows, the simulator harness suspends the next Resume request while its spinner is visible, then Withhold retires that active request. The picker must end its spinner with the readable prerequisite error (not genuine-empty copy), preserve both rows, and keep Retry reachable. Restore plus Retry must clear the error and refresh the rows successfully.
