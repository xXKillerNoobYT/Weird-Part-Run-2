...
- [x] PR #1422 **merged to main** (squash 606bfa29, 2026-07-07) after **six Copilot review rounds — 17 findings, all fixed**: rounds 1–2 (9: RETURNING removal, pairing host-mode closes on consume/dismiss, stale code cleared, auto-start gate = business profile OR company_id, BT→LAN pairing fallback, hint grammar, envelope docs), round 3 (4: blob omission + part_image_features de-whitelist, id-less row skip, conflict NULL propagation, applyUpdate NULL-bug copy), round 4 (2: ORDER BY rowid deterministic full-sync paging, unused property), round 5 (1: bonus.id ?? 0 silent no-op save), round 6 (1: over-broad negative scan → regex anchored to the regressing expression)
...
- [x] Wave-2 PR 1 **merged** (#1424, squash e58b1461, 2026-07-07): destructive-confirm helper (count-aware + named-record + defensive quoting, 16 behavioral tests) + full Orders pass — 94 craft sites, context menus on all 9 list pages (was 0), 10 destructive upgrades, live Procurement VoiceOver-swallowing bug fixed, `stableAccessibilitySuffix` shared helper, adoption contract tests. Six Copilot rounds, 13 findings (all cosmetic), round 6 clean.
- [x] Wave-2 PR 2 **merged** (#1425, squash 9954c3ab, 2026-07-07): Settings pass, 36 files — 102 accessibility sites, **DB-reset final confirmation layer** (was: wipe fired straight from the PIN screen; contract test pins PIN gate + final alert), Job Stage Templates unguarded destructive actions fixed, 7 discard dialogs standardized, context menus on 8 pages, blank-record-name fallback hardened at the helper. Six Copilot rounds, 16 findings, all cosmetic.
- [x] Wave-2 PR 3 — **merged** (#1426, squash 9041d4e6, 2026-07-07): Fleet craft pass, audible driver selection, stable identifiers, row accessibility, and context-menu second paths.
- [x] Wave-2 PR 4: People destructive sweep (15 count-less alerts)
- [x] WEI-4547 draft-import slice (2026-07-08): `WiredPart.dc.html` remains auth-gated outside interactive Claude Design, so this pass imported the known Panel Schedule Builder improvement rather than guessing from a hidden draft: `PanelQualityInstructionBanner` centralizes the visible mode/review guidance pattern, keeps 44pt/VoiceOver/stable-id behavior, replaces the panel move-mode one-off banner, and adds the same guidance to Sync Conflict Review without downgrading the existing critical choose-local/choose-remote flow.
- [x] WEI-4928 Catalyst critical-choice repair (2026-07-16): focused diagnostics isolated two interacting SwiftUI faults: Catalyst pointer activation reached the expanded AX wrapper without firing the bordered row button, and the recyclable row-owned alert did not present after the pending state changed. The choices now keep `dsMinTapTarget()` inside the native labels, route button/pointer/default-accessibility actions to one page-owned pending decision, and use the review page's single alert presenter for destructive confirmation and resolve failures. Focused local/remote UI tests prove Cancel retains Unit Cost and Confirm removes it without an error on Catalyst 1024×768 (Light local 63.14×53.90 AX pt; Dark remote 63.14×53.90), iPhone class (80×54.33), and iPad class (80×54); all six tests passed with confirmation/cancel/post-confirm screenshot attachments in the exact result bundles.
