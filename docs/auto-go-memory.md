...
- [2026-05-16] **getPreBillingData excludes locked billing periods**: WEI-362 / GH#362 resolved the 2026-04-19 gap. `getPreBillingData` still reports jobs + labor entries by requested date range, but now excludes labor entries covered by a non-deleted `billing_periods` row where `locked_at IS NOT NULL` and either `job_id` matches the labor entry job or `job_id IS NULL` for a company-wide lock.
