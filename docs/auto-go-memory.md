...
- [2026-04-19] **getPreBillingData locked-period gap resolved by WEI-743 / GitHub #362**: The earlier note was accurate at the time: pre-billing queried jobs + labor_entries by date range and did not use `billing_periods`. WEI-743 added a read-side guard so labor entries inside locked billing-period windows are excluded from pre-billing rows, with regression coverage for mixed locked/unlocked periods.
