...
- [x] **2026-04-26 — Investigate stale Copilot branches.** **DONE 2026-04-27T16:18Z (iter 12).** Two orphan branches investigated. (1) `copilot/fix-bugs-in-code` (commit `e2883649`, 2026-04-26T00:56Z): **no PR ever associated** (`gh pr list --state all --head` returned `[]`), commit verified not an ancestor of `origin/main` or any other ref → genuinely orphaned per owner rule → **deleted** via `git push origin --delete copilot/fix-bugs-in-code`. (2) `copilot/refactor-sensitive-field-encryption` (commit `48e3d4b9`, 2026-04-26T01:18Z): **has closed PR [#318](https://github.com/xXKillerNoobYT/Weird-Part-Run-2/pull/318)** ([WIP] Refactor and unify sensitive-field encryption, closed 2026-04-26T01:23Z, never merged) → backreference exists per owner rule → **kept**. Closed PR preserves diff history through GitHub's PR interface; safe to leave the branch indefinitely or revisit if the encryption refactor work resumes.
...
- [ ] **2026-04-27 — `docs/DevTODO/` hygiene: archive closed entries to `done/` subfolder.** Surfaced during inventory C11b lifecycle scan (iter 9). Of 19 DIS-*.md files in `docs/DevTODO/`, 13 are closed (`status: CLOSED`/`DONE`/`fixed`/`done`/`closed`/`partially-fixed`/`PARTIALLY_DONE`) and 5 are open. Mixing makes scanning + reasoning about active queue slower. **Proposed action:** create `docs/DevTODO/done/` (parallels `xcode-ai/fix-prompts/done/` pattern) and `git mv` all status:CLOSED|DONE|fixed entries there. Partial-status (DIS-015, DIS-017) stays in active dir. Owner ratification: is this a clean-up the loop should run autonomously, or do you want to keep closed entries inline as a permanent ledger? — _agent-discovered, awaiting owner answer_
