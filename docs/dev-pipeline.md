...
| Tests | **1255/1255 passing** — test-coverage-maintenance (2026-04-15): **2 production bugs fixed + 14 new tests.** (1) `findOverrideConflicts` SQLite arg-count mismatch (`brand_id IS NOT NULL` has no `?` but got one arg — fixed with `(SQL, argCount)` tuple approach); (2) `listLocationStockTargets` `v.name` → `v.vehicle_name`. Methods newly covered: `findOverrideConflicts`, `getPreviewParts`, `recalculateWeightedAvgCost`, `setBrandSuppliers`, `listForecastData`, `listForecastDataWithStock`, `saveForecastSettings`, `listAllForecastSettings`, `recalculateForecasts`, `listLocationStockTargets`. Previous: page-rebuild-enforcer (2026-04-14): 2 security fixes + 4 tests (#184 JSON injection + #191 unauthenticated key exchange). | 2026-04-15 |
