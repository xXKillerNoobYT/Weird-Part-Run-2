...
> **Last updated:** 2026-04-10 (dev-improvement-scanner run 12 — **DIS-016 NEW HIGH BUG:** 7 iOS write-op files use `currentUser?.id ?? 1` (admin ID fallback) — worse than DIS-015's `?? 0` because `1` is the admin's real ID; unauthenticated writes are attributed to admin. Files: IOSMessageThreadView, IOSCustomerDetailPage ×2, IOSContractorDetailPage, IOSProcurementPage, IOSNotebookDetailPage, IOSAuditSetupView. GitHub #140 filed. DevTODO DIS-016 created with Xcode AI prompt. All other scanners clean: 0 force casts, 0 force unwraps, 0 fatalErrors, 0 empty catches, SQL injection guarded via allowedFields allowlist, no secrets in UserDefaults. DIS-015 `?? 0` fully resolved — 3 remaining read-only instances acceptable. Tests: 1162 passing.)
...
| Tests | **1162/1162 passing** — test-coverage-maintenance (2026-04-10): +20 new tests covering WarehouseService, OrdersService (holdJPOLineWithChat), DashboardService (processQRScan ×5). Previous: 1142 (2026-04-09). | 2026-04-10 |
| Plan Alignment | **plan-enforcer run 11** (2026-04-10): DIS-015 write-path fixes verified in code. QRScannerAdapter SQL fix confirmed plan-aligned (qr_plan.md schema). No new drift detected. | 2026-04-10 |
| Feature Polish | **1 active DevTODO: DIS-016** (7 `?? 1` write-path files — GitHub #140). 0 active Xcode prompts. DIS-012/013 still blocked on KDF decision. DIS-014 ✅ CLOSED. DIS-015 ✅ CLOSED. | 2026-04-10 |
| Xcode Prompts | **0 active.** DevTODO DIS-016 has Xcode AI prompt — use it next. | 2026-04-10 |
| GitHub Issues | **~73 open** — #140 NEW (DIS-016: `?? 1` in 7 write-op files). #139 CLOSED (DIS-015). Open: #133 (PricingOverrideFlow no plan), #135-#137 (silent failures campaign), #138 (Cart Mode). Security: #130 (PIN KDF), #131 (legacy salt). Systemic: #121-#123, #128-#129. Program-review: #67-#95. | 2026-04-10 |
| Q&A Backlog | **4 pending** — DIS-012/013 (PIN KDF, blocked), #133 (PricingOverrideFlow plan — keep/remove?), #138 (Cart Mode priority + batch vs per-bin). | 2026-04-09 |
| Working Tree | ⚠️ Uncommitted changes from multiple sessions (PE-040/PE-041 impls, migration 073, DIS-014 fix, test additions, DIS-015 full write-path fixes, QRScannerAdapter SQL fix, +20 new tests, DIS-016 DevTODO) — for github-sync-and-review to commit. | 2026-04-10 |
...
| #140 | DIS-016: `currentUser?.id ?? 1` anti-pattern in 7 write-op iOS files (admin ID fallback) — DevTODO + Xcode AI prompt at `docs/DevTODO/DIS-016-hardcoded-userid-one-fallback.md` | 10 — Xcode prompt ready | 🔴 **HIGH** — Use Xcode AI prompt in DevTODO to fix all 7 locations | Xcode AI |
| #133 | PricingOverrideFlow.swift (616 lines) — no plan document, unplanned addition — Q&A generated (keep/remove/defer?) | 3 — Q&A asked | 🔲 Awaiting owner answer in `docs/dev-qa.md` | owner decision |
| #134 | WishlistService auto-approvals fix — `processAutoApprovals()` never fires (DIS-006 partial fix incomplete) | 13 — pending commit | ⚠️ Fix in working tree; issue still OPEN. `github-sync-and-review` must commit + close. | commit |
| #135 | [Usability] CompanySetupWizard `try?` on `saveSetupDraft` silently loses wizard progress on app switch | 2 — plan needed | 🔲 Part of systemic silent failures campaign (#128). Needs Q&A + plan before fix. | plan + Q&A |
| #136 | [Usability] IOSNotebookDetailPage `try?` on `createSection` returns nil — orphaned notebook entries | 2 — plan needed | 🔲 Part of systemic silent failures campaign (#128). Needs Q&A + plan before fix. | plan + Q&A |
| #137 | [Usability] IOSMessageThreadView `try?` on `autoSaveToJobNotebook` — attachment in chat but not in job notebook | 2 — plan needed | 🔲 Part of systemic silent failures campaign (#128). Needs Q&A + plan before fix. | plan + Q&A |
| #138 | Cart Mode service gap — `saveUnitPlacement` + `moveBinsToArea` not in WarehouseService — Q&A generated | 3 — Q&A asked | 🔲 Awaiting owner answers in `docs/dev-qa.md` | owner decision |
...
> **Phase 3 active.** Q&A: 4 pending (DIS-012/013 — KDF; #133 PricingOverrideFlow; #138 Cart Mode). Tests: 1126/1126. **0 active Xcode prompts.** PE-041 ✅ DONE + PE-040 ✅ DONE.

1. **Answer DIS-012/013 Q&A** — `docs/dev-qa.md` — 2 questions about PIN KDF upgrade (PBKDF2 vs Argon2id) and legacy salt path removal. No code can proceed until owner decides.
2. **Commit working tree** — `github-sync-and-review` — Large set of uncommitted changes: PE-040/PE-041 impls, migration 073, DIS-014 fix, #134 WishlistService processAutoApprovals fix, test additions. Also needs to close #134 after commit.
3. **Answer #133 Q&A (PricingOverrideFlow)** — `docs/dev-qa.md` — Decide: keep as official feature (write retroactive plan), or remove the 616-line file. Blocking plan registry entry.
4. **Answer #138 Q&A (Cart Mode)** — `docs/dev-qa.md` — Decide priority and batch vs per-bin movements. Blocks Cart Mode implementation in PE-030 remainder.
5. **Improve test coverage** — PeopleService (~22/47), ChatService (~14/33), SettingsService (~17/40).
6. **Program-review issues #82–#95** — 14 open page rebuild specs. Next wave of page rebuilds.
7. **Systemic silent failures #128 campaign** — #121 (198 `try?`), #122 (426 guard-let bails), #128 (empty catches), #129 (dirty tracking), now also #135-#137 (3 specific silent failures in CompanySetupWizard/IOSNotebookDetailPage/IOSMessageThreadView). Need plan + Q&A before automated fixes begin.
...
> Last updated by plan-enforcer: 2026-04-10 (run 11)
...
| `ios-warehouse-setup-redesign.md` | Warehouse | Step 11 (partial — Cart mode gap) | **Partial** — Setup tiers ✅, two flows ✅, CartManager ✅. `WizardStepPlacement.swift` **PE-040 DONE 2026-04-08** — dimensions-first form (rows×cols steppers → Confirm Grid → persists `grid_rows`/`grid_cols` via migration 073), Phase B `.onDrag`/`.dropDestination` drag-and-drop grid. **Remaining gap:** `saveUnitPlacement` and `moveBinsToArea` from plan not yet in WarehouseService — Cart mode flow is unimplemented. | PE-040 DONE; Cart mode still partial |
...
| `hunt-fix-verify-loop.md` | Quality | Step 7 | **Full** — 36 iterations, 128 bugs fixed. Latest run 36 (2026-04-10): DIS-015 all 6 write-path `?? 0` fixed. Plan Alignment scanner confirmed running (plan-enforcer run 11). | Active — DIS-012/013 still blocked on KDF design |
...
| [#138](https://github.com/xXKillerNoobYT/Weird-Part-Run-2/issues/138) | [Plan Drift] Warehouse Setup Cart Mode — `saveUnitPlacement` + `moveBinsToArea` not implemented | Plan Gap | Step 3 — Q&A asked | Q&A generated 2026-04-09 — priority + batch-vs-per-bin decision needed. | 🟡 Open — awaiting owner Q&A |
...
|-------|----------|-------------|-------------|--------|
| hunt-fix-verify | 2026-04-08 (run 34) | DIS-014 CLOSED — legacy unsigned token shim removed from `parseLocalToken`. `AppDatabase.schemaVersion` 61→74. Migration 073 test added. +3 tests: `testParseTokenRejectsUnsigned`, `testParseTokenRejectsTamperedSig`, `testMigration073FloorPlanGridDimensions`. 1121/1121. | 3 direct fixes, 3 tests added | ✅ Healthy |
| test-coverage-maintenance | 2026-04-08 | +5 tests: `updateFloorPlanGrid` (3 — PE-040), `cancelJPOLineTransfer` (2). 1126/1126 passing. | 5 tests added | ✅ Healthy — gaps: PeopleService (~22/47), ChatService (~14/33), SettingsService (~17/40) |
| plan-enforcer | 2026-04-08 (run 9) | PE-040 + PE-041 confirmed done. Migration 073 registered. Schema version = 74. Cart Mode gap filed as #138. | Plan registry updated, 1 issue filed | ✅ Healthy |
| dev-improvement-scanner | 2026-04-07 (run 10) | DIS-006 incomplete fix found and completed in `IOSWishlistPage.loadData()` (processAutoApprovals background Task). #134 filed. Full scan clean. | 1 direct fix, 1 issue filed | ✅ Healthy |
| dev-pipeline-manager | 2026-04-09 (run 15) | 6 new issues processed (#133-#138). Q&A generated for #133 (PricingOverrideFlow) + #138 (Cart Mode). Active work table + GitHub issues table + Next Up updated. | 5 Q&A questions added, 14 new issue rows in table | ✅ Healthy |
