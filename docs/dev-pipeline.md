...
> **Last updated:** 2026-04-17 (dev-improvement-scanner run 15 — 8 direct fixes, 4 GitHub issues filed, 1302/1302 passing)
...
|------|--------|-------------|
| Build | 0 errors, 0 warnings | 2026-04-17 |
| Tests | **1302/1302 passing** (55 suites) — dev-improvement-scanner run 15 (2026-04-17): 1 new regression test (`calculateSupplierScores_onTimeRateWithCompletedSession`). Previous: test-coverage-maintenance (2026-04-17): 1301/1301. | 2026-04-17 |
| Plan Alignment | **plan-enforcer run 15 (2026-04-17):** Full working tree + plan registry audit. **1 tracker drift fixed:** PE-044 marked DONE — `AddEmployeeSheet` in `IOSEmployeesPage.swift` has full dismiss guard since commit 7024173 (2026-04-15); `00-fix-order.md` updated, PE-045 stub added for remaining Phase 1A People/HR sheets. **5 uncommitted changes verified plan-aligned:** PartsService `'complete'`→`'completed'` SQL fix + atomic `recalculateAllSupplierScores`, ToolsService DRY `allowedToolEditFields` extraction, ConflictResolver+PeerManager os.log upgrade (#128/#121), dev-qa.md #148 Q&A cleanup. **1 partial implementation:** `EditEmployeeContactSheet` (IOSEmployeeDetailPage) uses `.interactiveDismissDisabled(isSaving)` not `(isDirty)` — wrong predicate per dismiss-safety plan. GitHub issue filed. **PE-COLORS (#234-#243) + #221 (LWW) confirmed correctly pending.** Q&A backlog: 0 pending (dev-qa.md #148 block in working tree → removed). | 2026-04-17 |
| Feature Polish | **2 active DevTODOs.** DIS-017 (sub-44pt tap targets — needs Xcode prompt), DIS-018 (PageHelpSheet detents — needs Xcode prompt). DIS-016 ✅ CLOSED. DIS-015 ✅ CLOSED. DIS-014 ✅ CLOSED. DIS-012/013 deferred to v2. | 2026-04-17 |
| Xcode Prompts | **1 active: PE-045 (NEXT — to be written).** PE-044 ✅ DONE (commit 7024173, 2026-04-15; verified plan-enforcer run 15). PE-045 covers remaining Phase 1A People/HR dismiss guards: EditEmployeeContactSheet (wrong predicate upgrade), AddCustomerSheet, AddContractorSheet, contact sheets. Plan: `docs/plans/dismiss-safety-campaign.md`. | 2026-04-17 |
| GitHub Issues | **34 open** — dev-improvement-scanner run 15 (2026-04-17) filed: #246 (tap targets < 44pt), #247 (LAN sync HTTP MITM + ATS fixed), #248 (sheet detents missing), #249 (refreshable missing 3 pages). Previous: 30 open (github-issues-sync run 9 — 2026-04-15). | 2026-04-17 |
| Q&A Backlog | **0 pending** (plan-enforcer run 15: #148 Q&A block removed — issue CLOSED, draft persistence shipped in `IOSMovementWizard.swift`). All April 2026 clusters processed 2026-04-14. Design decisions in: `docs/plans/colors-parts-redesign.md`, `docs/plans/dismiss-safety-campaign.md`, `docs/plans/april-2026-audit-closures.md`, `docs/plans/sync-field-timestamps-upgrade.md`, `docs/plans/pagination-cutover.md`. | 2026-04-17 |
| Working Tree | ⏳ **Uncommitted changes (11 files)** — plan-enforcer run 15 (2026-04-17): PartsService `'complete'`→`'completed'` SQL fix + atomic `recalculateAllSupplierScores`; ToolsService `allowedToolEditFields` DRY; ConflictResolver + PeerManager + FoundationModelsService + IOSClockPage `try?`→do-catch + os.log (#128/#121); `PartsServiceInventoryTests` regression test; IOSAutoFillBanner accessibility label; `.gitignore` heartbeat patterns; dev-qa.md #148 Q&A removed. All plan-aligned. Push pending github-sync-and-review. | 2026-04-17 |
...
| PE-DISMISS | #143 Dismiss Safety — per-sheet `@State isDirty` + `.onChange` watchers + `.interactiveDismissDisabled(isDirty)` + Discard-changes alert. Phase 1A People/HR first, 1B Chat, 1C Orders/Fleet/Scheduling, 1D Parts/Tools/Settings last. **PE-044 ✅ DONE 2026-04-15** (commit 7024173 — `AddEmployeeSheet` in `IOSEmployeesPage.swift` full pattern). #149 (scrollDismissesKeyboard) slots AFTER #143 completes as separate Phase 2. | 10 — PE-044 done, PE-045 next | ✅ **PE-044 DONE.** **#245 filed 2026-04-17** — `EditEmployeeContactSheet` wrong predicate (`isSaving` not `isDirty`). **PE-045 stub:** write prompts for EditEmployeeContactSheet + AddCustomerSheet + AddContractorSheet + contact sheets using PE-044 as reference. | Xcode AI |
...
| 2026-04-17 | **plan-enforcer run 15**: Full plan registry + working tree audit. **1 tracker drift fixed:** PE-044 marked DONE in `00-fix-order.md` — `AddEmployeeSheet` (IOSEmployeesPage.swift) has full dismiss guard since commit 7024173 (2026-04-15). **PE-045 stub added** for remaining Phase 1A People/HR sheets. **7 uncommitted working-tree changes verified plan-aligned:** PartsService `receiving_sessions.status='complete'`→`'completed'` SQL bug + atomic `recalculateAllSupplierScores`; ToolsService `allowedToolEditFields` DRY; ConflictResolver + PeerManager + FoundationModelsService `os.log` upgrade + `try?`→do-catch (#128/#121); `PartsServiceInventoryTests` regression test; dev-qa.md #148 Q&A removed (CLOSED). **1 new issue filed:** #245 — `EditEmployeeContactSheet` uses `.interactiveDismissDisabled(isSaving)` not `(isDirty)`. **Correctly pending:** PE-COLORS (#234-#243), #221 (LWW). **Q&A: 0 pending.** 6 new Plan Registry entries added. | Steps 11-12 | 00-fix-order.md + dev-pipeline.md |
...
| `testing-strategy.md` | Quality | Step 7 (fine-tuned) | **Full** — **1290/1290 passing** (hunt-fix-verify run 11, 2026-04-17). 55 suites. Coverage gaps: PeopleService (~22/47), ChatService (~14/33). #229 resolveConflicts iOS UI tests still pending (Xcode scope). | Updated run 15 |
...
| `dismiss-safety-campaign.md` | Cross-cutting/UX | Step 10 | **Partial** — **PE-044 ✅ DONE** (AddEmployeeSheet). #245 filed (EditEmployeeContactSheet wrong predicate). Phase 1A remaining: PE-045 pending for ~4 sheets. Phase 1B-D, Phase 2 (#149) pending. | Added run 15 |
| `sync-field-timestamps-upgrade.md` | Sync | Step 2 | **Missing** — `FieldTimestampHelper.swift` not created, `_field_timestamps` column not in any migration. #221 OPEN. | Correctly pending — large blast radius |
| `pagination-cutover.md` | Quality/Perf | Step 5 | **Partial** — Phase 1 band-aid done: `BaseRepository.findAll()` defaults to `limit: 1000`. Phase 2 (call-site audit) + Phase 3 (cursor pagination) pending. #223/#227 OPEN. | Phase 1 code confirmed run 15 |
| `colors-parts-redesign.md` | Parts | Step 2 | **Missing** — `color_brand_skus` table not in migrations. Issues #234-#243 OPEN. Phase 1 (schema+service) not started. | Correctly pending — major schema change |
| `april-2026-audit-closures.md` | Quality | Step 5–7 | **Partial** — #224 (ADU filter ✅ DONE, CLOSED), #221 (pending, see sync-field-timestamps-upgrade.md), #223/#227 (Phase 1 done, see pagination-cutover.md). | Updated run 15 |
...
| [#36](https://github.com/xXKillerNoobYT/Weird-Part-Run-2/issues/36) | Receiving back button discards work with no confirmation | Bug | Step 13 | FIXED/SUPERSEDED (PE-041, verified 2026-05-26) — owner chose autosave instead of a confirm dialog; `IOSReceiveShipmentPage.swift` persists quantity mutations and restores drafts on reopen. | 🟢 Closed |
...
| plan-enforcer | 2026-04-17 (run 15) | Full plan registry + working tree audit. PE-044 tracker drift fixed (marked DONE). PE-045 stub added. 5 uncommitted changes verified plan-aligned. #245 filed (EditEmployeeContactSheet wrong dismiss predicate). Q&A backlog cleared to 0. Plan Registry updated with 6 new entries. | Plan registry updated, #245 filed | ✅ Healthy |
