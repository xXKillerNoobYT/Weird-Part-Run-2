...
> **Iteration 96 (2026-07-07):** people — **#1429 filed: People add-sheet save failures reported as load errors**. 1 logical fix (6 UI save catch blocks + 1 source-scan regression). Focused on the latest People destructive sweep (#1428) and adjacent add-sheet error surfacing. Six People create sheets used `userFriendlyError(error, context: "load ...")` inside save/create catch blocks, so failed customer/contractor/employee/hat/team/contact writes displayed misleading load-failure copy. Fix: changed contexts to `save customer`, `save contractor`, `save employee`, `save hat`, `save team`, and `save contact`. Added `SilentLoadFailureSurfacingRegressionTests.testPeopleAddSheetSaveFailuresUseSaveSpecificErrorContexts` to pin the save-specific copy and reject stale load snippets. Verification: focused local Mac xcodebuild test passed for the new regression (`WiredPart-iOS`, WEI3988-iPhone simulator). Files: `IOSCustomersPage.swift`, `IOSContractorsPage.swift`, `IOSEmployeesPage.swift`, `IOSHatsPage.swift`, `IOSTeamsPage.swift`, `IOSContactsPage.swift`, `SilentLoadFailureSurfacingRegressionTests.swift`. Build ✅ via focused iOS test.
>
> **Iteration 95 (2026-04-25):** fleet — **#277 closed: silent-data-loss on Report Vehicle Issue**. 1 logical fix (4 sites: migration + service + UI + test). 1738/1738. ReportVehicleIssueSheet's Submit button just called `onComplete()` — user-typed severity + description vanished with no service write, no error, no retry. Pure silent data loss in the My Truck driver-reporting flow. Fixes: (1) Migration 077 — new `vehicle_issue_reports` table (vehicle_id FK→vehicles cascade, reported_by FK→users, severity TEXT, description TEXT, status TEXT default 'open', deleted_at, timestamps; +2 indexes on vehicle_id and status). (2) `FleetService.reportVehicleIssue(vehicleId:reportedBy:severity:description:)` — FK-guards both vehicle (`deleted_at IS NULL AND is_active = 1`) and reporter (`deleted_at IS NULL`); validates severity against `validIssueSeverities` set (low/medium/high/critical); rejects empty/whitespace-only descriptions; trims before INSERT. New `FleetError.invalidIssueReport(String)` enum case. (3) `IOSMyTruckPage.ReportVehicleIssueSheet` — added `vehicleId: Int64` parameter, `@EnvironmentObject AppCore`, `isSaving`/`saveError` state, `submit()` flows userId from `appCore.currentUser?.id` (no hardcoded 1), surfaces saveError in a Section, disables Cancel/Submit during save, ContentUnavailableView fallback when no vehicle assigned. (4) `testReportVehicleIssue` — happy path + 5 negative cases (empty desc, whitespace-only desc, invalid severity, missing vehicle FK, missing user FK, tombstoned vehicle). DB verification reads back the row to confirm trimmed description and default `status='open'`. **Closed GitHub #277.** Files: `AppDatabase+Migrations.swift`, `FleetService.swift`, `IOSMyTruckPage.swift`, `FleetServiceTests.swift`. Build ✅ — tests 1738/1738 (+1).
>
> **Iteration 93 (2026-04-19):** 133 iterations, 618 bugs fixed. inventory — **WarehouseService consolidation + rating tombstone guards + PartsService is_active + listPerLocationForecastData**. 7 fixes + 7 tests. 1653/1653. Focus area: inventory (AUTO GO rotated from vehicles). **Hunt class: soft-delete + FK constraint mismatch.** SQLite's `ON DELETE CASCADE` fires on hard-delete only — tombstoned (soft-deleted) parent rows still satisfy FK existence checks. WarehouseService's consolidation + rating write paths trusted FK constraints to catch tombstoned references, but 5 paths silently persisted orphan rows against deleted users/areas/votes. Fixes: (1) `castConsolidationVote` — userId + chosenAreaId tombstone guards before `ConsolidationVoteEntry.insert` (the INSERT succeeds against a tombstoned user/area because FK only checks row existence, not deleted_at; the subsequent vote tally is corrupted by ghost entries). (2) `updateUserRating` — userId tombstone guard before the SELECT-or-INSERT-then-UPDATE pattern; previously minted orphan `user_warehouse_ratings` rows for tombstoned users, and those rows never clean up via cascade because users are soft-deleted. (3) `recordOrgCheck` — areaId + checkedBy dual tombstone guards; `organization_ratings.area_id` (FK→warehouse_storage_areas) and `last_org_check_by` (FK→users) both need explicit soft-delete checks. (4) `managerOverrideConsolidation` — `UPDATE consolidation_votes WHERE id = ?` gained `AND deleted_at IS NULL`; tombstoned votes could be resurrected by manager override before the fix, flipping status from deleted to 'decided'. (5) `dismissConsolidation` — same `AND deleted_at IS NULL` guard on UPDATE. (6) `PartsService.createPart` — `record.isActive = 1` explicit assignment so GRDB's INSERT doesn't override the `parts.is_active` DB default with NULL; without this, `listForecastDataWithStock` (which filters `p.is_active = 1`) silently excluded every part created through the service. Pre-existing failure surfaced today: `testListForecastDataWithStockEmpty` was receiving `rows → []` despite seeded parts. (7) `PartsService.listPerLocationForecastData()` — new method listing all non-deleted `location_stock_targets` ordered by `part_id, location_type`; compile-blocking test reference `env.parts.listPerLocationForecastData()` (used by `testRecalculateForecastsPerLocationSkipsInactiveParts`) now resolves. `isTableNotFoundError` graceful fallback applied. 7 regression tests in WarehouseFloorPlanTests: `testCastConsolidationVote_rejectsTombstonedUser`, `testCastConsolidationVote_rejectsTombstonedArea`, `testUpdateUserRating_rejectsTombstonedUser`, `testRecordOrgCheck_rejectsTombstonedArea`, `testRecordOrgCheck_rejectsTombstonedChecker`, `testManagerOverrideConsolidation_noOpOnTombstonedVote`, `testDismissConsolidation_noOpOnTombstonedVote`. Each tombstone test uses the raw-SQL-UPDATE pattern (set `deleted_at = datetime('now')` on seeded entity, then call the guarded method) — directly exercises the exact DB state the guard defends against. Files: `WarehouseService.swift`, `PartsService.swift`, `WarehouseFloorPlanTests.swift`. — remaining 8 — build ✅ — tests 1653/1653
>
> **Iteration 92 (2026-04-19):** 132 iterations, 611 bugs fixed. vehicles/warehouse — **#260 closed: getAllPartConfidenceLevels() batch query**. 1 fix + 1 test. 1642/1642. Focus area: vehicles (AUTO GO rotated). FleetService write-path sweep was already converged — all 7 write paths had guards from iters 47/51/59/73/81. Pivoted to GitHub #260: IOSAuditPage loaded confidence data via 11 separate queries (one per reliability level 0–10). Fix: added `getAllPartConfidenceLevels()` to WarehouseService — `PartConfidence.fetchAll(dbConn)` with `ORDER BY reliability_level ASC, confidence_percent DESC`, wrapped in `isTableNotFoundError` graceful fallback. The function is a drop-in batch replacement for the 11-loop pattern in IOSAuditPage. Regression test: `testGetAllPartConfidenceLevels_returnsAllLevels` seeds 2 parts at different confidence levels, calls `getAllPartConfidenceLevels()`, and asserts the total matches the sum of all 11 per-level calls — verifying the batch is semantically equivalent. **Closed GitHub #260.** Remaining 10 open (all XCODE_ONLY, program-review, or systemic UI). Files: `WarehouseService.swift`, `WarehouseFloorPlanTests.swift`. — remaining 10 — build ✅ — tests 1642/1642
>
> **Iteration 91 (2026-04-19):** 131 iterations, 610 bugs fixed. tools — **ToolsService validation sweep: ToolsError enum + returnTool/checkoutTool/checkoutToolWithCondition/returnToolWithCondition/createMaintenanceConfig guards**. 5 fixes + 8 tests. 1638/1638. Focus area: tools (AUTO GO rotated from people). Key findings: (1) `ToolsError` enum added — `toolNotFound(Int64)`, `userNotFound(Int64)`, `requiredFieldEmpty(String)` — ToolsService was the only remaining write-heavy service with no dedicated error enum; callers could only catch generic GRDB errors. (2) `checkoutTool` — added `users.deleted_at IS NULL` tombstone guard for `userId`; tool tombstone guard existed (iter 57) but the `INSERT INTO tool_checkouts (checked_out_by = userId)` was unguarded — a tombstoned user would orphan-FK the `checked_out_by` column. (3) `returnTool` — had zero guards at all; added tool existence check (`toolNotFound` on tombstoned/missing tool) + user tombstone guard (`userNotFound`); without these, the `UPDATE tool_checkouts SET checked_in_by = userId` INSERT would write a tombstoned user FK, and the `UPDATE tools SET status = 'available'` would silently no-op on a missing tool with no error. (4) `checkoutToolWithCondition` — added blank `condition` guard **before** the transaction; `conditionToRating("")` falls through to the `default: return 4` ("Good") branch — a blank condition records the tool as "Good" in `tool_checkouts.checkout_condition` and sets `condition_rating = 4`, creating a false positive audit trail entry. The guard fires outside `db.writer.write` because it's a pure input check (no DB state needed). (5) `returnToolWithCondition` — added blank `condition` guard + tool tombstone guard; same silent-data-corruption class as checkoutWithCondition, plus the function had no tool existence check (UPDATE matched zero rows silently). (6) `createMaintenanceConfig` — added blank `type` guard + tool tombstone guard; a config against a tombstoned tool is invisible to `getMaintenanceConfigs` (filtered by `AND tool_id = ?`) but still occupies a `tool_maintenance_configs` row; blank `maintenance_type` is unclassifiable by any maintenance-type filter. 8 regression tests: `testCheckoutTool_rejectsTombstonedUser`, `testReturnTool_rejectsTombstonedTool`, `testReturnTool_rejectsTombstonedUser`, `testCheckoutToolWithCondition_rejectsBlankCondition` (empty + whitespace), `testReturnToolWithCondition_rejectsBlankCondition`, `testReturnToolWithCondition_rejectsTombstonedTool`, `testCreateMaintenanceConfig_rejectsBlankType` (empty + whitespace), `testCreateMaintenanceConfig_rejectsTombstonedTool`. Files: `ToolsService.swift`, `ToolsServiceTests.swift`. — remaining 11 — build ✅ — tests 1638/1638
>
> **Iteration 90 (2026-04-19):** 130 iterations, 605 bugs fixed. people + scheduling — **updatePaymentSettings guards + getLongTermTimeline N+1 batch rewrite (#261 closed)**. 2 fixes + 2 tests. 1630/1630. Key findings: (1) `PeopleService.updatePaymentSettings` — `termsDays > 0` guard; calling `updatePaymentSettings(termsDays: 0, ...)` would store `"0"` as `default_payment_terms_days`, making every new invoice due_date == issue_date — immediately triggering overdue checks. `warningDays < 0` guard prevents nonsensical threshold. `warningDays = 0` passes (warn on the day itself — valid business rule). Uses `PeopleError.invalidAmount(Double(termsDays))` since the value represents a duration, not a payment amount, but `invalidAmount` is the most semantically appropriate existing case. (2) `SchedulingService.getLongTermTimeline` — the original implementation called `getJobsForMonth` + `getPendingBidCountForMonth` in a 36-iteration loop (72 queries total). Batched into 2: `batchFetchJobsInRange(from:to:)` fetches all overlapping jobs in one query (with `start_date` and `due_date` columns); `batchFetchBidCounts(from:to:)` uses `GROUP BY STRFTIME('%Y-%m', start_date)` for per-month bids + separate null-start count. In-memory overlap check replaces per-month SQL filter. New private `JobTimelineRow` struct needed to carry dates for the filter without polluting the public `JobSummary` API. Key edge cases preserved: nil `due_date` (ongoing job) → sentinel `"9999-12-31"` matches any month; null `start_date` bids → counted in every month by adding to all month totals. Test: `testLongTermTimeline_batchJobOverlap` creates a job spanning month M and M+1, verifies it appears in both months' job lists. **Closed GitHub #261.** Remaining 11 (all XCODE_ONLY or program-review). Files: `PeopleService.swift`, `SchedulingService.swift`, `PeopleServiceTests.swift`, `SchedulingServiceTests.swift`. — remaining 11 — build ✅ — tests 1630/1630
>
> **Iteration 89 (2026-04-19):** 129 iterations, 603 bugs fixed. people — **createContact/updateContact/updateTeam blank guards + createPaymentRecord/recordPayment amount+date guards**. 5 fixes (+1 enum case) + 9 tests. 1628/1628. Focus area: people. Key findings: (1) `PeopleError.invalidAmount(Double)` added — semantically distinct from `invalidScore`; payment amounts need their own error case to distinguish "you passed an out-of-range contractor score" from "you passed a negative invoice amount". (2) `createContact` — blank `entityType`/`firstName` guards; `entity_contacts` uses a polymorphic `entity_type`/`entity_id` pattern with no DB-level FK constraint (SQLite can't enforce polymorphic FKs), so the app-layer guard is the only line of defense; a blank `entityType` creates contacts invisible to all entity-type-scoped queries. (3) `updateContact` — blank `firstName` guard; the update path had `AND deleted_at IS NULL` on the WHERE clause (already correct) but no input guard — blank first_name would render as " Doe" in every contact list and detail view. (4) `updateTeam` — blank `name` guard on the update path (the `createTeam` path had the guard since iter 81, but `updateTeam` was missed); a team renamed to '' appears as an empty cell on job cards in the dispatch board. (5) `createPaymentRecord` — `amount > 0` + blank `dueDate` guards; zero-amount invoice has `amount - paid_amount = 0` so it never appears in `getOverdueCustomers` (which filters on `status = 'overdue'` + amount > paid); blank `dueDate` stores `due_date = ''` and all overdue date-comparison queries silently treat it as immediately overdue. (6) `recordPayment` — `amount > 0` + blank `paidDate` guards; zero-amount payment is a no-op that sets `status = 'partial'` (since `0 >= invoiceAmount` is false), creating false partial-pay status noise; blank `paidDate` stores `paid_date = ''` which corrupts payment-history date-range reports. 9 regression tests: `testCreateContact_rejectsBlankFields`, `testUpdateContact_rejectsBlankFirstName`, `testUpdateTeam_rejectsBlankName`, `testPaymentRecord_rejectsInvalidInputs` (6 cases: zero/negative amount + blank date for both create + record). Files: `PeopleService.swift`, `PeopleServiceTests.swift`. — remaining 12 — build ✅ — tests 1628/1628
>
