...
## Status: PASS — 44/44 pages complete. Run 2026-04-14: **#184 + #191 FIXED** — (1) JSON injection in `LanSyncServer.checkAuth` cert rejection response (replaced string interpolation with JSONEncoder); (2) Unauthenticated `GET /sync/key` endpoint now requires `X-Company-ID` header; `PeerManager` updated to send it. 4 new tests. 1241/1241 passing. **Working tree includes:** Parts pages dismiss-guard additions (#143 sweep), CartManager @MainActor (#185), forecasting/export SQL fixes (#188, #209), PartsModels isActive fields, ChangeTracker OOM LIMIT (#190 partial), ConflictResolver upsert-on-missing (#220), encryptIfNeeded fail-closed (#230), AuthService SecItemAdd warning. Previous run 2026-04-12 (PM): **#146 major sweep** — 9 new cached formatters added to Formatters.swift (`iso8601DateOnly`, `mediumDateTimeFormatter`, `timeHHmmFormatter`, `fullDateFormatter`, `dayOfWeekFormatter`, `shortDateDisplayFormatter`, `sqlDateTimeFormatter`, `monthDayFormatter`, `monthDayYearFormatter`). **37 inline instances eliminated** across 9 files (IOSPODetailPage ×7, IOSScheduleConfigPage ×6, DashboardView ×5, IOSWishlistPage ×4, TimelinePriorityColor ×3, IOSSubSchedulePage ×3, IOSDispatchPage ×3, PartsForecastingPage ×3, IOSWeeklyReviewSheet ×3). Also fixed IOSSubSchedulePage formatter-mutation bug (same instance mutated from parse→display format — replaced with two separate cached formatters). 46 non-Formatters instances remain across ~28 files (all ≤2 per file). 1217/1217 tests pass.
