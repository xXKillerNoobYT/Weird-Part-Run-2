...
struct IOSClockPage: View {
    ...
    @State private var activeSupplyRunStart: Date?
    ...
    private func clockedInSection(_ entry: JobsService.LaborEntryRow) -> some View {
        Section("Current Status") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                if activityStatus == "supply_run", let activeSupplyRunStart {
                    activeSupplyRunCard(start: activeSupplyRunStart)
                }
    ...
    private func toggleSupplyRun(entryId: Int64) async {
        ...
        do {
            ...
            let start = JobsService.activeSupplyRunStart(notes: notes)
            await MainActor.run {
                activityStatus = newStatus
                activeSupplyRunStart = start
    ...
    private func updateSupplyRunElapsedText() {
        guard let activeSupplyRunStart else {
            supplyRunElapsedText = ""
            return
        }
        let elapsed = Date().timeIntervalSince(activeSupplyRunStart)
        let hours = Int(elapsed) / 3600
        let minutes = (Int(elapsed) % 3600) / 60
        ...
        do {
            ...
            await MainActor.run {
                ...
                if let brk = currentBreak {
                    ...
                    activeSupplyRunStart = currentSupplyRunStart
                    updateSupplyRunElapsedText()
                    breakTimer?.invalidate()
                    breakTimer = nil
                    breakElapsedText = ""
