...
struct PanelSchedulePDFExporter {
    ...
    private func renderPDF() -> Data {
        ...
        return renderer.pdfData { context in
            ...
            y += 14
            drawPanelInfo(margin: margin, y: &y)
            y += 10
            drawScheduleTable(in: pageSize, margin: margin, y: &y)
    ...
    private func drawPanelInfo(margin: CGFloat, y: inout CGFloat) {
        let titleAttributes: [NSAttributedString.Key: Any] = [
    ...
    private func drawScheduleTable(in pageSize: CGSize, margin: CGFloat, y: inout CGFloat) {
        ...
        for row in 0..<(schedule.totalSpaces / 2) {
            ...
            drawCircuitRow(spaceNumber: leftSpace, x: leftX, width: halfWidth, y: y, attributes: cellAttributes)
            drawCircuitRow(spaceNumber: rightSpace, x: rightX, width: halfWidth, y: y, attributes: cellAttributes)
            y += rowHeight
        }
    }
