# Fix Prompt 12F: Fast Continuous QR Scanner

> **BEFORE DOING ANYTHING:** Read `xcode-ai/xcode.md` and follow every instruction in it.
>
> **DEPENDS ON:** Prompt 12A must be completed (routing). Independent of 12C/12D/12E.

---

## What the User Wants

The QR Scanner should be FAST. When you navigate to Dashboard > QR Scanner, the camera opens immediately. As you point it at different QR codes, the info card updates in real-time showing what you're looking at. A "Lock" button lets you freeze on one item to see full details or take an action. Quick actions auto-lock the camera until the action is done. The last scanned item is remembered if the camera loses sight of the QR code.

---

## File To Edit

**`Weird Parts IOS/Features/Dashboard/IOSDashboardQRScannerPage.swift`**

### Complete Rewrite

The current page has a manual-first approach (camera button + text field). Replace with a camera-first continuous scanner.
...
struct IOSDashboardQRScannerPage: View {
    ...
    private var cameraSection: some View {
        ZStack {
            ...
            // For now, show a simulated viewfinder
            Rectangle()
                .fill(Color.black)
                .overlay(
                    VStack {
    ...
    private var noCameraFallback: some View {
        ScrollView {
            VStack(spacing: DS.Space.xl) {
                ...
                // Manual entry (full-width version)
                VStack(alignment: .leading, spacing: DS.Space.sm) {
                    Text("Manual Entry")
        ...
        Task {
            do {
                ...
                for await event in stream {
                    guard !isLocked else { continue } // Skip events when locked
    ...
    // MARK: - Helpers (keep existing implementations)
...
1. Navigate to Dashboard > QR Scanner → camera opens immediately (no tap required)
2. Point at a QR code → info card appears at bottom showing item name, type, stock locations
3. Point at a different QR → info card updates instantly to show the new item
4. Move camera away from QR → last scanned item stays visible (remembered)
5. Tap Lock button → camera pauses, full details + quick actions appear
6. Tap a quick action (e.g., "Move Stock") → camera stays locked, action runs
7. Tap "Resume Scanning" or "Scan Next" → camera resumes, info card clears
8. Manual entry field at the bottom works as fallback
9. If camera is unavailable (Mac, simulator), shows manual-only fallback

---

## All Dashboard Hub Prompts (12A-12F) Complete

Run them in order: **12A → 12B → 12C → 12D → 12E → 12F**

After all 6 are done, the Dashboard will be:
- **Overview:** KPIs + charts + clock status banner + alerts + quick actions
- **Clock:** Inline GPS-sorted job picker + shop/job linking + geofence monitoring
- **Daily Report:** Personal hours + team status + fast actions + pending/deliveries/budgets
- **QR Scanner:** Instant continuous camera scanner with lock/unlock + contextual actions
