...
|---|------|---------------|--------|
| PE-045 | *(to be written)* | **Dismiss Guard: Phase 1A remaining People/HR sheets** — EditEmployeeContactSheet (IOSEmployeeDetailPage — currently uses wrong `.interactiveDismissDisabled(isSaving)` predicate, needs upgrade to `isDirty`), AddCustomerSheet (IOSCustomersPage), AddContractorSheet (IOSContractorsPage), AddContactSheet / EditContactSheet (IOSContactsPage). Pattern: reference PE-044. Plan: `docs/plans/dismiss-safety-campaign.md`. GitHub #143. | 🔲 **NEXT** — write prompt(s) first |
