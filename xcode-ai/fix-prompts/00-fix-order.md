...
| PE-045 | *(to be written)* | **Dismiss Guard: Phase 1A remaining People/HR sheets** — ~~EditEmployeeContactSheet (IOSEmployeeDetailPage)~~ ✅ done (upgraded from `isSaving` to `isDirty` pattern, 2026-04-18), AddCustomerSheet (IOSCustomersPage), AddContractorSheet (IOSContractorsPage), AddContactSheet / EditContactSheet (IOSContactsPage). Pattern: reference PE-044. Plan: `docs/plans/dismiss-safety-campaign.md`. GitHub #143. | 🔲 **DISMISS NEXT** — write prompt(s) first |
