# 16E — Price Override Confirmation Flow

> **Chain position:** 16A → 16B → 16C → 16D → **16E** → 16F (bulk) → 16G–16I
> **Prerequisite:** 16D complete (pricing page rebuilt with tier system)
...
When a user sets a price at a hierarchy level (e.g., Category), parts below that level with NO existing override get the new price automatically. But parts that DO have a lower-level override keep their existing price — the user must CONFIRM each override replacement ONE AT A TIME.

**Critical rule:** There is NO "confirm all" or batch confirmation. Each override point is reviewed individually. The user sees the old price, new price, and dollar difference, then decides: Replace or Keep.

This is separate from the preview feature (16F). The preview shows a sample of AFFECTED parts (read-only visualization). The override flow handles CONFLICTING tiers (actionable, one at a time).

**Key files:**
- Create new file: `Weird Parts IOS/Weird Parts IOS/Features/Parts/PricingOverrideFlow.swift`
- Modify: `Weird Parts IOS/Weird Parts IOS/Features/Parts/PartsPricingPage.swift` (add trigger)

## Task

### Step 1: Create `PricingOverrideFlow.swift`

This view handles setting a price at a hierarchy level. It's presented as a sheet with multiple steps:
1. Pick the level (Category, Style, Type, Brand)
2. Pick the specific entity at that level
3. Enter the new markup/margin
4. See read-only preview of 15 random affected parts
5. If overrides exist, step through them ONE AT A TIME
...
### Step 2: Add trigger in PartsPricingPage.swift

Add a new case to `PricingActiveSheet`:
...
Update the `id` computed property:

```swift
case .setTierPricing: return "tier"
```

### Step 3: Add list methods if missing

If `PartsService` doesn't have `listStyles()`, `listTypes()`, or `listBrands()` that return flat lists, add them. Check first — they likely exist from earlier work.

## Success Criteria

- [ ] 5-step flow: Select Level → Select Entity → Set Price → Preview → Resolve Conflicts → Done
- [ ] Preview shows up to 15 random affected parts (READ ONLY, no action buttons)
- [ ] Override conflicts reviewed ONE AT A TIME — no batch confirm
- [ ] Each conflict shows: level, current sell, new sell, difference, parts affected
- [ ] Two options per conflict: "Replace Override" or "Keep Override"
- [ ] Progress bar shows N of M during conflict resolution
- [ ] After all conflicts resolved, tier is saved and replacements are applied
- [ ] Done screen shows counts (X replaced, Y kept)
- [ ] Accessible from pricing page toolbar menu
...
```
## Prompt 16E Results (YYYY-MM-DD)
- Created PricingOverrideFlow.swift with PricingTierSetSheet
- 5-step flow: level → entity → price → preview → conflicts → done
- Override confirmation: strictly one-at-a-time with Replace/Keep
- Build: [PASS/FAIL]
```

**Wait for user confirmation before proceeding to prompt 16F.**
