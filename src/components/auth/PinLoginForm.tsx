...
export function PinLoginForm({
  ...
  // DEV ONLY: auto-fill PIN for iOS Simulator testing
  // (WKWebView doesn't receive keyboard events from Mac host)
  useEffect(() => {
    if (import.meta.env.DEV) {
      const t = setTimeout(() => {
        if (pin === '') {
          console.log('[DEV] Auto-filling PIN for simulator testing');
          setPin('1234');
        }
      }, 2000);
  ...
  return (
    <div className="min-h-screen bg-surface-secondary flex flex-col items-center justify-center p-6">
      ...
      {/* PIN input — visible but styled minimally so iOS WebView can focus it */}
      <input
        ref={inputRef}
        type="password"
        inputMode="numeric"
        ...
        className="w-48 h-12 text-center text-2xl tracking-[0.5em] border border-gray-300 dark:border-gray-600 rounded-lg bg-transparent focus:outline-none focus:ring-2 focus:ring-primary-500"
        autoComplete="off"
