...
| Issue reference | `Closes #N` (GitHub issue number) **must** appear in the PR body when a GitHub issue exists. `WEI-XXXX` (Paperclip) is **additive only** — include it for traceability but it does **not** substitute the GitHub `#N` reference. Example: `Closes #998` + `Related: WEI-3578` |
