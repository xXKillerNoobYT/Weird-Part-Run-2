...
checkout_pr_branch() {
  local number="$1" branch="$2" expected_head="$3" action="$4" fetched_head
  git fetch origin "$BASE" "$branch"
  fetched_head="$(git rev-parse "origin/$branch" 2>/dev/null || true)"
  if [[ -z "$fetched_head" ]]; then
    echo "    skip: could not resolve fetched PR branch head before $action — not acting on unknown head"
    return 1
  fi
  if [[ "$fetched_head" != "$expected_head" ]]; then
    echo "    skip: fetched PR branch head changed from ${expected_head:0:8} to ${fetched_head:0:8} before $action — not repairing stale review/check state"
    return 1
  fi
...
commit_and_push_if_changed() {
  local number="$1" mode="$2" expected_head="$3" branch="$4"
  if [[ -n "$(git status --porcelain)" ]]; then
    ...
    run_or_log git push --force-with-lease="refs/heads/$branch:$expected_head" origin "HEAD:refs/heads/$branch"
    return 0
  fi
...
while IFS= read -r pr; do
  ...
  if [[ "$mergeable" == "CONFLICTING" || "$merge_state" == "DIRTY" ]]; then
    ...
    if ! checkout_pr_branch "$number" "$head_branch" "$head_sha" "$mode repair"; then
      continue
    fi
    set +e
    ...
    commit_and_push_if_changed "$number" "$mode" "$head_sha" "$head_branch"
    exit 0
  fi
