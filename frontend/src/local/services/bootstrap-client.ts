 ...
 * management for Capacitor mobile shells.
 *
 * Responsibilities:
 ...
 * (Capacitor), files are saved into the app's data directory via
 * @capacitor/filesystem. Both paths share the same verification logic.
 *
 * Architecture:
...
async function stateSet(key: string, value: string): Promise<void> {
  if (isNative) {
...
async function stateRemove(key: string): Promise<void> {
  if (isNative) {
 ...
 * Works identically on both web and Capacitor (both have crypto.subtle).
 */
 ...
 *
 * Uses fetch + ReadableStream which works on both web and Capacitor's
 ...
 *
 * On native platforms, uses @capacitor/filesystem to write into the
 * app's data directory.  On web, creates an object URL for download.
 *
...
async function saveFile(
  ...
  if (isNative) {
    try {
      // Dynamic import — @capacitor/filesystem is only available on native
      // @ts-expect-error -- @capacitor/filesystem only available in native builds
      const { Filesystem, Directory } = await import('@capacitor/filesystem');

      // Convert Uint8Array to base64 for Capacitor's writeFile
 ...
 * Used for Capacitor Filesystem which expects base64 data.
 */
