 ...
 * secure key storage for Capacitor (iOS/Android).
 *
 * Responsibilities:
 * 1. Generate and store the device's Ed25519 keypair in the OS keystore
 * 2. Store device certificate securely (not in plain localStorage)
 ...
 *
 * Architecture:
 * - iOS: Uses Keychain via @capacitor/preferences with AccessGroup
 * - Android: Uses Android Keystore via @capacitor/preferences
 * - Web: In-memory only (no persistent secure storage needed)
 */
 ...
 *
 * On native platforms (iOS/Android), uses @capacitor/preferences which
 * stores data in the app's secure container:
 * - iOS: NSUserDefaults (sandboxed, encrypted at rest by iOS Data Protection)
 * - Android: SharedPreferences (sandboxed, encrypted at rest on modern Android)
 *
 ...
 *
 * Uses Web Crypto API which is available in both browser and Capacitor.
 */
