/**
 * Local Relay Service — peer-to-peer data relay for Capacitor devices.
 *
...
async function relaySet(key: string, value: string): Promise<void> {
  if (isNative) {
...
async function relayGet(key: string): Promise<string | null> {
  if (isNative) {
...
async function relayRemove(key: string): Promise<void> {
  if (isNative) {
