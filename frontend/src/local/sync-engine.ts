 ...
 *
 * Runs in Capacitor mode only. Triggered by:
 * - App launch / resume
 * - Manual "Sync Now" button
 * - Periodic timer (every 5 minutes when online, backoff when failing)
 * - Network change detection (@capacitor/network)
 * - App state change (foreground resume)
 *
 * Protocol:
...
export async function setupNetworkListener(deviceId: string): Promise<void> {
  ...
  try {
    ...
    // @capacitor/network not available (e.g., browser mode) — skip
  }
}
