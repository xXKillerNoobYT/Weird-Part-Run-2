 ...
 * approve/reject when the user has manage_orders permission.
 */
...
import { listJPOs, bulkApproveOrReject } from '../../../api/orders';
...
export function PartsRequestsPage() {
  ...
  const bulk = useBulkSelection(filteredJPOs);

  // Bulk approve mutation
  const bulkApproveMut = useMutation({
    mutationFn: () =>
      bulkApproveOrReject({
        items: [...bulk.selectedIds].map((id) => ({ entity_type: 'jpo' as const, entity_id: id })),
        action: 'approve',
      }),
    onSuccess: () => {
      bulk.clear();
  ...
  const hasPendingSelected = bulk.selectedItems.some((j) => j.status === 'pending_approval');
  const bulkLoading = bulkApproveMut.isPending || bulkRejectMut.isPending;
  ...
  const bulkActions: BulkAction[] = [
    {
      ...
      show: canManage && hasPendingSelected,
    },
    {
      label: 'Reject',
      icon: X,
          ...
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-surface-secondary">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400 hidden sm:table-cell">Requested By</th>
            ...
            <tbody className="divide-y divide-border">
              {filteredJPOs.map((jpo) => (
                ...
                >
                  {canManage && (
                    <BulkCheckbox
                      checked={bulk.isSelected(jpo.id)}
                      onChange={() => bulk.toggle(jpo.id)}
                    />
                  )}
