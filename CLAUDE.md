...
**Pacing (updated 2026-08-02 — FULL AUTO, MODEL-AWARE guide):** AUTO GO must NOT self-limit for budget reasons; the owner sets limits in chat only. **The running agent must know which LLM model it is on and pick its pacing guide accordingly (owner 2026-08-02):** a **Fable/Mythos session paces on the Fable-scoped weekly window** (Fable has its own limit right now; 100% of it used by the Monday reset), while an **Opus 5/4.8/other session paces on the OVERALL weekly** (no Opus-specific limit currently). Same shape either way: steady work all week on a linear ramp landing at 100% AT the reset — maximum usage, never an early stop. Run the pacer with the model passed explicitly: `CLAUDE_PACE_MODEL=<your model id> ~/.claude/scripts/claude-pace.sh` (or `--model`; `python3 ~/.claude/usage-pacer.py` accepts the same) — the agent knows its model from its own system prompt; with no model passed the pacer defaults to the overall bucket, which is the safe wall every model stops at. Verdicts from delta vs the ramp: < −12 BURN, < −4 PUSH, < 3 STEADY, else EASE OFF. The non-guide buckets print as info — a per-model bucket sitting low is capacity that expires unused, so route hard work to that model rather than slowing down. **Daily floor** unchanged: at least ONE AUTO GO iteration per day. The pacer auto-starts the "Usage for Claude" app when its local API (127.0.0.1:47291) is down; Fable window = `/usage` → `extraWindows[key=="win_weekly_scoped_fable"]`, Opus = `weeklyLimits.opusPercentage`, reset timing = `/weekly`. Fallbacks: Paperclip cost ledger (URL-encode timestamp colons), then the transcript estimator. Crons remain wake-up mechanisms only; trigger phrases (`AUTO GO`, `HUNT FIX`, `AUTO GO STOP/RESUME`) still work via `~/.claude/hooks/auto-go-trigger.sh`.
