...
**Pacing (updated 2026-08-01 — FULL AUTO, Fable bucket is THE guide):** AUTO GO must NOT self-limit for budget reasons. No budget_mode, no self-imposed fire caps, no "budget-aware" throttling baked into the loop. The owner sets any pace limit in chat, explicitly, when they want one; absent such a directive the loop runs full auto. Weekly-limit *timing* (not limiting) is handled centrally. **Owner directive 2026-08-01: the pacing guide is the FABLE-scoped weekly window, and 100% of Fable must be used by the reset** — a straight linear ramp to 100% at Monday 10:00 AM local, with **no owner reserve on this bucket** (this supersedes the 2026-07-30 overall-weekly model and the 2026-07-31 15% reserve for pacing decisions; the owner acknowledged the conflict in the same message). The overall all-models weekly is still reported as an info line, and when its remaining headroom is smaller than Fable's remaining need the pacers print a WARNING that the overall wall arrives first (all models stop there) — informational only, never a brake. Every AUTO GO iteration checks the pacer (`~/.claude/scripts/claude-pace.sh`, or `python3 ~/.claude/usage-pacer.py` for the deep view); verdicts from delta vs the Fable ramp: < −12 BURN, < −4 PUSH, < 3 STEADY, else EASE OFF (land at 100% AT reset, not before). **Daily floor** unchanged: at least ONE AUTO GO iteration per day regardless of verdict (pacing shifts iteration size and delay, never to zero). The pacer auto-starts the "Usage for Claude" app if its local API is down. Pacer sources, in order: (1) **ClaudeUsage macOS app local API** `http://127.0.0.1:47291` — the Fable window comes from `/usage` → `extraWindows[key=="win_weekly_scoped_fable"].percentage`, reset timing from `/weekly` (same instant); read-only, app must be running with Local API enabled; if the Fable window is missing from `/usage`, fall back loudly to the overall weekly and report it; (2) **Paperclip cost ledger cross-check** — the Providers tab at `http://127.0.0.1:3100/<company>/costs`, API `/api/companies/<cid>/costs/by-provider` summed across all companies (URL-encode timestamp colons or it silently returns `[]`); (3) transcript-based estimator fallback when the app is down. The historical 6h/3h-offset cron design (2026-04-23 "budget-aware default") is retired as a *limit*; crons remain merely a wake-up mechanism, and each fire still does ONE focused iteration with clean heartbeat handoff. Trigger phrases still work typed directly (`AUTO GO`, `HUNT FIX`, `AUTO GO STOP/RESUME`) via `~/.claude/hooks/auto-go-trigger.sh`.
